export async function DeleteOldData(db) {
    ...
    await db.query(`
        ...
        JOIN (
            SELECT hosting_id,
                   DATE_SUB(MAX(data_i_czas), INTERVAL 7 DAYS) AS granica
    ...
    await db.query(`
        ...
        JOIN (
            SELECT u2.hosting_id,
                   DATE_SUB(MAX(r2.data_i_czas), INTERVAL 7 DAYS) AS granica
    ...
    await db.query(`
        ...
        JOIN (
            SELECT u2.hosting_id,
                   DATE_SUB(MAX(h2.data_i_czas), INTERVAL 7 DAYS) AS granica
    ...
    await db.query(`
        ...
        WHERE data_i_czas < (
            SELECT granica FROM (
                SELECT DATE_SUB(MAX(data_i_czas), INTERVAL 7 DAYS) AS granica
    ...
    await db.query(`
        ...
        AND data_zamkniecia < (
            SELECT granica FROM (
                SELECT DATE_SUB(MAX(data_utworzenia), INTERVAL 7 DAYS) AS granica
                FROM ALARMY
            ) tmp
        )
    `);
